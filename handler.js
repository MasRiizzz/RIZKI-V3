...
if (chat.welcome) {
	...
	for (let user of participants) {
		...
		if (action == 'add') {
			buf = await this.getBuffer(`https://api.dapuhy.xyz/api/canvas/welcome3?name=${m.name}&gcname=${groupMetadata.subject}&member=${participants.lenght}&pp=${pp}&bg=https://i.postimg.cc/Kc0ChTtc/31367.jpg&apikey=4oYjiJ6vJr`)
			this.sendFile(id, buf, '', '', null, false, {mentions: [user] })
		}
		if (action == 'remove') {
			buff = await this.getBuffer(`https://api.dapuhy.xyz/api/canvas/goodbye?name=${m.name}&gcname=${groupMetadata.subject}&ppgc=${this.profilePictureUrl(groupMetadata.id, 'image')}&pp=${pp}&bg=https://i.ibb.co/tYgwwT2/images-2.jpg&apikey=4oYjiJ6vJr`)
